    ...
    public function index() {
        ...
        if (!empty($this->session->userdata('validlogin')) || !empty($_REQUEST ['GUID']) ) {
            ...
            if(isset($req->error) && ($req->error=='faultError')){
                ...
                $this->email->to(array('ksivanadiyan@asce.org','cvalliere@asce.org'));
                    ...
                    if(isset($ReqMasterId->error) && ($ReqMasterId->error=='faultError')){
                       ...
                       //         $getIp = $this->LoginModel->getIp($ip);
                                ...
                                if ($ip <= $high_ip && $low_ip <= $ip) {
                                    ...
                                    $msg = "You do not have access to any active licenses, please contact ASCE Customer Service for assistance at 1-800-548-2723 or 1-703-295-6300, 8:00 am - 6:00 pm ET or <a href='mailto:ascelibrary@asce.org'>ascelibrary@asce.org</a>";
                                    if ($count == 'N') {
                           ...
                           //             $getIp = $this->LoginModel->getIp($ip);
                              ...
                              //          $getIp = $this->LoginModel->getIp($ip);
                                        ...
                                        if ($ip <= $high_ip && $low_ip <= $ip) {
                                            ...
                                            $this->session->set_userdata(array('MasterCustomerId' => $MasterCustomerId, 'LabelipName' => $UserName, 'ip' => $ip));
                                            //print_r( $_SESSION); die;
                                            $msg = "You do not have access to any active licenses, please contact ASCE Customer Service for assistance at 1-800-548-2723 or 1-703-295-6300, 8:00 am - 6:00 pm ET or <a href='mailto:cvalliere@asce.org'>cvalliere@asce.org</a>";
                                            if ($count == 'N') {
                                                $this->session->set_flashdata('message', $msg);
                                                $data ['productList'] = $this->ProductModel->get_book();
                                                $this->template->load('pagetemplate/header', 'ProductList');
                                    ...
                                    // echo "hello"; die;	      
                                    $data ['subscribedProductList'] = $this->ProductModel->get_Subscribed_Product($MasterCustomerId);
                                    //echo "<pre>"; print_r($data ['subscribedProductList']); die("Hell");
                                    $fname = $data ['subscribedProductList'] [0]->m_firstname;
                                    $lname = $data ['subscribedProductList'] [0]->m_lastname;
                                    $orderid = $data['subscribedProductList'] [0]->m_orderid;
                                    $fulname = $fname . " " . $lname;
