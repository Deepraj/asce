...
class Tank_auth
{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->ci->session->set_userdata(array(
								...
								'role'		=>	(count($user->m_rolname) == 0 ? "publisher" : $user->m_rolname)
						));
	...
	function is_logged_in($activated = TRUE)
	{
        ...
        /**
	 * Check if user logged as super administrator. Also test if user is activated or not.
	 *
	 * @param	bool
	 * @return	bool
	 */
        function is_logged_in_admin($activated = TRUE)
	{
                $Book_Type = $this->ci->session->userdata('book_type');
                if(!empty($Book_Type)){
	...
	{
	 return ($this->ci->session->userdata('role') == "publisher" ? true : false);

		if($this->ci->session->userdata('role') == "student"){
			$this->error = array('login' => 'student login try again later');
		}
	}
	function is_user_student()
	{
	 return ($this->ci->session->userdata('role') == "student" ? true : false);	
	}
	...
	function create_user($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			$data = array(
				...
				'm_usrroleid'	=> $this->ci->users->get_student_roles_id()
			);
	...
	function change_email($email)
	{
		...
		if(is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
			...
			if (strtolower($email) == strtolower($email)){		// leave activation key as is
				$data['new_email_key'] = $this->ci->session->userdata('new_email_key');
				return $data;
