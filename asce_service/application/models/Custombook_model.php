...
class Custombook_model extends CI_Model
{
	...
	// Function to search a custom book
	function custom_booksearch($search,$from_date,$to_date){
		
		$this->db->select ('m_custboktitle,m_custbokdescription,m_custbokprice,m_bokisbn,m_volnumber,m_custbokthumb,m_custbokvalidfrom,m_custbokvalidto,m_boktitle,m_custbokcreatedby' ); 
	...
	function show_custombook($user_id,$search,$from_date,$to_date){
		...
		if($search == '' && $from_date == '' && $to_date == ''){	
			$this->db->select ('m_custboktitle,m_custbokid,m_custmbokid,m_custbokdescription,m_custbokprice,m_custbokvalidfrom,m_custbokvalidto,m_custbokthumb,m_volnumber,m_bokisbn,m_boktitle,m_custbokcreatedby' ); 
			$this->db->from ('m_custbook');
			$this->db->join ( 'm_volume', 'm_volume.m_volbokid = m_custbook.m_custmbokid' , 'inner' );
			$this->db->join ( 'm_book', 'm_bokid = m_volbokid' , 'inner' );
			//$this->db->where(array('m_custbokcreatedby' =>$user_id));
			$this->db->order_by("m_custbokcreateddate", "desc");
			$query = $this->db->get();
			...
			//$this->db->like("(m_custboktitle like '%" . addslashes($search) ."%' or m_custbokdescription like '%" . addslashes($search) ."%' or m_custbokprice like '%" . addslashes($search) ."%' )");
			$this->db->or_like('m_custboktitle',$search);
			$this->db->or_like('m_custbokdescription',$search);
			$this->db->or_like('m_custbokprice',$search);
			$this->db->from ('m_custbook');
			//$this->db->where('m_custbokvalidfrom >=', $from_date);
			//$this->db->where('m_custbokvalidto <=', $to_date); 
			$this->db->join ( 'm_volume', 'm_volume.m_volbokid = m_custbook.m_custmbokid' , 'inner' );
	...
	// Function to get a volume number
	...
	/*function list_coverimage($bookid){
		
		$this->db->select ('m_bokthump'); 
		$this->db->from ('m_book');
		$this->db->join ( 'm_volume', 'm_volume.m_volbokid = m_book.m_bokid' , 'inner' );
		$this->db->join ( 'm_chapter', 'm_chpbokvid = m_volid' , 'inner' );
		$this->db->where(array('m_bokid' =>$book_id));
	...
	// Function to add a custom chapter
	...
	// Function to edit a custom book
	...
	// Function to edit a custom chapter
	...
	// Function to update a custombook
	...
	// Function to delete a custom chapter
